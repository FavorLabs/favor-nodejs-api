...
exports.getInfo = asyncHandler(async (req, res, next) => {
    let user = await UserDetail.findById(req.user._id,{invitations:1,activation:1,code:1});
    user = user.toObject()
    user.vilad = await UserDetail.countDocuments({invitation:user.code,activation:{$gt:1000}})
